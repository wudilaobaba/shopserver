...
import com.whj.shop.shopserver.logic.OrderChecker;
import com.whj.shop.shopserver.modelReal.OrderEntity;
import com.whj.shop.shopserver.util.CommonUtil;
import com.whj.shop.shopserver.vo.OrderIdVO;
import com.whj.shop.shopserver.vo.OrderPureVO;
import com.whj.shop.shopserver.vo.OrderSimplifyVO;
...
public class OrderController {
    ...
    public OrderPureVO getOrderDetail(@PathVariable(name = "id") Long oid) {
        Optional<OrderEntity> orderOptional = this.orderService.getOrderDetail(oid);
    ...
    public PagingDozer getUnpaid(@RequestParam(defaultValue = "0")
        ...
        Page<OrderEntity> orderPage = this.orderService.getUnpaid(page.getPage(), page.getCount());
    ...
    public PagingDozer getByStatus(@PathVariable int status,
                                   ...
                                   @RequestParam(name = "count", defaultValue = "10")
                                           Integer count) {
        PageCounter page = CommonUtil.converToPageParameter(start, count);
        Page<OrderEntity> paging = this.orderService.getByStatus(status, page.getPage(), page.getCount());
